 ...
 */
abstract class DB_Table extends dbModel\DB_Model implements CRUD\tableQuery
{
    /**
     * @param int $errorCode
     *
    ...
    protected function process_insert($tableName, array $columnNames, array $columnValues)
    {
         ...
         */
        $prepareQuery = new PrepareStatementInsert($columnNames, $columnValues);
...
//            self::$errorInfo = $insertStmt->errorInfo();
    ...
    protected function process_update($tableName, array $updateValues = null, array $updateRow = null)
    {
        ...
        /**
         * CREATE WHERE CLAUSE, PARAMETERS AND BINDINGS FIRST
         */
        $namedParam = $prepareQuery->getPreparedNamedParameters('Where');
        $bindings = $prepareQuery->getBindings();
        $whereClause = $prepareQuery->getWhereClause();

        /**
         * CREATE UPDATE PARAMETERS AND BINDINGS
         */
        $updateParam = $prepareQuery->getUpdateParam();
        $updateBindings = $prepareQuery->getBindings();
        $updateClause = $prepareQuery->getUpdateClause();

        if ($updateClause != "" && $whereClause != "")
            $updateQuery = "UPDATE {$tableName} SET {$updateClause} {$whereClause}";
        else
            return -1;

        $finalBindings = array_merge($updateBindings, $bindings);
        ...
        try {
            ...
            $updateStmt->execute($finalBindings);
...
//            self::$errorInfo = $deleteStmt->errorInfo();
    ...
    protected function process_select($tableName, array $selectRow = null, $selectColumns = "*", $className = "")
    {
        ...
        if ($selectRow != null) {
            ...
            $bindings = $prepareQuery->getBindings();
            $whereClause = $prepareQuery->getWhereClause();
            //TODO: CHANGE ORDER BY -> DYNAMICALLY
            if ($whereClause != "")
//                $selectQuery = "SELECT {$selectColumns} FROM {$tableName} {$whereClause} ORDER BY event_id";
                $selectQuery = "SELECT {$selectColumns} FROM {$tableName} {$whereClause}";
            else {
...
//            $selectQuery = "SELECT {$selectColumns} FROM {$tableName} ORDER BY event_id";
        ...
        try {
            ...
            //TODO: THINK ABOUT SINGLE RESULT SETS
//            if (count($result) == 1)
//                return $result[0];
